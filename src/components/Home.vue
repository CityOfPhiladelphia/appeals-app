      ...
      <div class="columns medium-16 text-center">
        <div class="card">
          <div class="card-divider">
            <h3
              v-show="!filtering"
            >There are {{ rowsCount }} appeals from {{ displayDate1 | readableDate }} to {{ displayDate2 | readableDate }}</h3>
            <h3 v-show="filtering">Fetching data...</h3>
          </div>
          <div class="card-section nopadding-xs">
            <custom-table
              v-on:rowClick="goToDetail"
              v-bind:enableRowClick="true"
              v-bind:allowLoadMore="true"
              v-bind:rows="localRows"
...
export default {
  ...
  beforeMount() {
    ...
    let forceURL = false;
    if (
      moment(this.$route.params.date1, VALIDATE_URL_DATE_FORMAT, true).isValid()
    ) {
      this.date1 = moment(
        this.$route.params.date1,
        VALIDATE_URL_DATE_FORMAT
      ).format(DATE_FORMAT);
    } else {
      // Wrong URL
      forceURL = true;
    ...
    if (
      moment(this.$route.params.date2, VALIDATE_URL_DATE_FORMAT, true).isValid()
    ) {
      this.date2 = moment(
        this.$route.params.date2,
        VALIDATE_URL_DATE_FORMAT
      ).format(DATE_FORMAT);
    } else {
      // Wrong URL
  ...
  mounted() {
    this.datepicker1 = $(this.$refs.refDate1)
      .fdatepicker({
    ...
    this.datepicker2 = $(this.$refs.refDate2)
      .fdatepicker({
        //initialDate: this.date2,
        format: "yyyy-mm-dd",
        onRender(date) {
          return date.valueOf() <= homeStore.datepicker1.date.valueOf()
            ? "disabled"
            : "";
        }
      })
